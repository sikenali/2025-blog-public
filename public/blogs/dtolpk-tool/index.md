...
> 用户体验分析：先分析这个源码的主要功能和用户需求，确定核心交互逻辑。   * 产品界面规划：作为产品经理，定义关键界面，确保信息架构合理，参考Beyond Compare 功能实现。   * 高保真 UI 设计：作为 UI 设计师，设计贴近真实 PC 设计规范的界面，使用现代化的 UI 元素，使其具有良好的视觉体验。   * 原型实现：使用 HTML + Tailwind CSS（或 Bootstrap）生成所有原型界面，并使用 FontAwesome（或其他开源 UI 组件）让界面更加精美、接近真实的 App 设计。拆分代码文件，保持结构清晰。
